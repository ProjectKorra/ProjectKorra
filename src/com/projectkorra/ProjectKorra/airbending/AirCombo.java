...
public class AirCombo {
	...
	public AirCombo(Player player, String ability) {
		...
		if (GeneralMethods.isRegionProtectedFromBuild(player, "AirBlast", player.getLocation())) return;
		...
		if (ability.equalsIgnoreCase("Twister")) {
			...
			for (double y = 0; y < height; y += TWISTER_HEIGHT_PER_PARTICLE) {
				...
				for (double i = -180; i <= 180; i += TWISTER_DEGREE_PER_PARTICLE) {
					...
					animLoc.add(0, y, 0);
					if (i % rand == 0) AirMethods.playAirbendingParticles(animLoc, 1, 0, 0, 0);
				}
			}
			...
			for (int i = 0; i < height; i += 3)
				for (Entity entity : GeneralMethods.getEntitiesAroundPoint(currentLoc.clone().add(0, i, 0), radius * 0.75))
					if (!affectedEntities.contains(entity) && !entity.equals(player)) affectedEntities.add(entity);
			...
			if (Math.abs(player.getLocation().distance(currentLoc)) > range) {
				remove();
				return;
			} else if (affectedEntities.size() > 0 && System.currentTimeMillis() - time >= AIR_STREAM_ENTITY_DURATION) {
				remove();
				return;
				...
				BukkitRunnable br = new BukkitRunnable() {
					...
					public void run() {
						for (int angle = -180; angle <= 180; angle += 45) {
							...
							if (angle % rand == 0) AirMethods.playAirbendingParticles(loc.clone().add(orthog), 1, 0F, 0F, 0F);
						}
					}
				};
	...
	public static ArrayList<AirCombo> getAirCombo(Player player, ClickType type) {
		ArrayList<AirCombo> list = new ArrayList<AirCombo>();
		for (AirCombo combo : instances)
			if (combo.player != null && combo.player == player && combo.type != null && combo.type == type) list.add(combo);
		return list;
	}
