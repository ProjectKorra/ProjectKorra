...
public class WaterMethods {
	...
	public static boolean canBeBloodbent(Player player) {
		if (AvatarState.isAvatarState(player)) if (ChiMethods.isChiBlocked(player.getName())) return true;
		if (GeneralMethods.canBend(player.getName(), "Bloodbending") && !GeneralMethods.getBendingPlayer(player.getName()).isToggled()) return false;
		return true;
	}
	 ...
	 * 
	 * @param player
	 *            the player that is attempting to Waterbend.
	 * @param range
	 *            the maximum block selection range.
	 * @param plantbending
	 *            true if the player can bend plants.
	...
	public static Block getWaterSourceBlock(Player player, double range, boolean plantbending) {
		...
		for (double i = 0; i <= range; i++) {
			...
			if (GeneralMethods.isRegionProtectedFromBuild(player, "WaterManipulation", location)) continue;
	...
	public static Block getIceSourceBlock(Player player, double range) {
		...
		for (double i = 0; i <= range; i++) {
			Block block = location.clone().add(vector.clone().multiply(i)).getBlock();
			if (GeneralMethods.isRegionProtectedFromBuild(player, "IceBlast", location)) continue;
	...
	public static Block getPlantSourceBlock(Player player, double range, boolean onlyLeaves) {
		...
		for (double i = 0; i <= range; i++) {
			Block block = location.clone().add(vector.clone().multiply(i)).getBlock();
			if (GeneralMethods.isRegionProtectedFromBuild(player, "PlantDisc", location)) continue;
	...
	public static boolean isAdjacentToFrozenBlock(Block block) {
		...
		for (BlockFace face : faces) {
			if (FreezeMelt.frozenblocks.containsKey((block.getRelative(face)))) adjacent = true;
		}
	 ...
	 *            The location to use
	 * @param radius
	 *            The radius around the location to remove spouts in
	 * @param source
