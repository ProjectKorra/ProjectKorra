...
public class WaterWave {
        ...
        if (type == AbilityType.CLICK) {
            ...
            if (anim == AnimateState.SHRINK) {
                ...
                double currentSpeed = speed
                        - (speed
                        * (double) (System.currentTimeMillis() - time) / flightTime);
                ...
                GeneralMethods.getBlocksAroundPoint(player.getLocation().add(0, -1, 0), waveRadius).stream()
                        .filter(block -> block.getType() == Material.AIR && !GeneralMethods.isRegionProtectedFromBuild(player, "WaterSpout", block.getLocation()))
                        .forEach(block -> {
                            if (iceWave)
                ...
                if (iceWave && progressCounter % 3 == 0) {
                    GeneralMethods.getEntitiesAroundPoint(player.getLocation().add(0, -1, 0), waveRadius * 1.5).stream()
                            .filter(entity -> entity != this.player && entity instanceof LivingEntity && !affectedEntities.contains(entity))
                            .forEach(entity -> {
    ...
    public static void progressAll() {
        instances.forEach(WaterWave::progress);
    }
    ...
    public static List<WaterWave> getType(Player player, AbilityType type) {
        return instances.stream()
                .filter(spear -> spear.player.equals(player) && spear.type.equals(type))
