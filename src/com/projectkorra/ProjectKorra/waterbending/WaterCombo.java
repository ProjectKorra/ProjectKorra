...
public class WaterCombo {
    ...
    public WaterCombo(Player player, String ability) {
        ...
        if (ability.equalsIgnoreCase("IceBulletLeftClick") || ability.equalsIgnoreCase("IceBulletRightClick")) {
            List<WaterCombo> bullets = getWaterCombo(player, "IceBullet");
            if (bullets.size() == 0)
                return;
    ...
    public void progress() {
        ...
        if (ability.equalsIgnoreCase("IceWave")) {
            ...
            if (origin == null) {
                ...
                Location startingLoc = GeneralMethods.getTopBlock(ent.getLocation().add(0, -1, 0), (int) range)
                        .getLocation();
                if (startingLoc == null) {
                    remove();
                    return;
    ...
    public void drawWaterCircle(Location loc, double theta, double increment, double radius, Material mat, byte data) {
        direction = GeneralMethods.rotateXZ(direction, theta);
    ...
    public static List<WaterCombo> getWaterCombo(Player player) {
