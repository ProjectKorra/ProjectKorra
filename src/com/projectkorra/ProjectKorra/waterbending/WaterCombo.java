...
public class WaterCombo {
	...
	private double damage = 0, speed = 0, range = 0, knockback = 0, radius = 0,
			shootTime = 0, maxShots = 0;
	...
	public WaterCombo(Player player, String ability) {
		...
		if (GeneralMethods.isRegionProtectedFromBuild(player, "WaterManipulation", player.getLocation())) return;
	...
	public void progress() {
		...
		if (ability.equalsIgnoreCase("IceWave")) {
			if (origin == null && WaterWave.containsType(player, WaterWave.AbilityType.RELEASE)) {
				if (bplayer.isOnCooldown("IceWave") && !AvatarState.isAvatarState(player)) {
					remove();
					return;
				}
				bplayer.addCooldown("IceWave", cooldown);
				origin = player.getLocation();
				WaterWave wave = WaterWave.getType(player, WaterWave.AbilityType.RELEASE).get(0);
				wave.setIceWave(true);
			} else if (!WaterWave.containsType(player, WaterWave.AbilityType.RELEASE)) {
				remove();
				return;
			}
		} else if (ability.equalsIgnoreCase("IcePillar")) {
			...
			if (origin == null) {
				if (bplayer.isOnCooldown("IcePillar") && !AvatarState.isAvatarState(player)) {
					remove();
					return;
				}
				origin = player.getLocation();
				...
				Location startingLoc = GeneralMethods.getTopBlock(ent.getLocation().add(0, -1, 0), (int) range).getLocation();
				if (startingLoc == null) {
					remove();
					return;
				}
				...
				for (double x = -radius; x <= radius; x++)
					for (double z = -radius; z <= radius; z++) {
						...
						if (tmpLoc.distance(startingLoc) > radius) continue;

						Block block = GeneralMethods.getTopBlock(tmpLoc, (int) range, (int) range);
						if (!WaterMethods.isWaterbendable(block, player)) badBlocks++;
					}
				...
				for (double x = -radius; x <= radius; x++)
					for (double z = -radius; z <= radius; z++) {
						Block block = currentLoc.clone().add(x, 0, z).getBlock();
						if (WaterMethods.isWaterbendable(block, player) || block.getType() == Material.AIR) if (block.getLocation().distance(currentLoc) > radius) continue;
						if (GeneralMethods.isRegionProtectedFromBuild(player, "WaterManipulation", block.getLocation())) continue;
			...
			if (origin == null) {
				if (bplayer.isOnCooldown("IceBullet") && !AvatarState.isAvatarState(player)) {
					remove();
					return;
				}
				Block waterBlock = BlockSource.getWaterSourceBlock(player, range, ClickType.LEFT_CLICK, true, true, WaterMethods.canPlantbend(player));
				if (waterBlock == null) {
					remove();
					return;
				}
				...
				if (this.state == AbilityState.ICE_BULLET_FORMING) {
					...
					if (timeDiff < shootTime) {
						if (shots < rightClicks + leftClicks) {
							...
							FireComboStream fs = new FireComboStream(null, vec, loc, range, speed);
							fs.setDensity(10);
							fs.setSpread(0.1F);
			...
			if (i % 2 == 0) {
				for (Entity entity : GeneralMethods.getEntitiesAroundPoint(loc, 1.5)) {
					...
					if (!entity.equals(player)) {
						...
						if (damage != 0) if (entity instanceof LivingEntity) GeneralMethods.damageEntity(player, entity, damage);
					}
				}
	...
	public void drawWaterCircle(Location loc, double theta, double increment, double radius, Material mat, byte data) {
		...
		for (double i = 0; i < theta; i += increment) {
			...
			if (block.getType() == Material.AIR && !GeneralMethods.isRegionProtectedFromBuild(player, "WaterManipulation", block.getLocation())) createBlock(block, mat, data);
		}
	}
	...
	public void remove() {
		...
		for (BukkitRunnable task : tasks)
			task.cancel();
		revertBlocks();
		if (waterGrabber != null) waterGrabber.remove();
	}
	...
	public static ArrayList<WaterCombo> getWaterCombo(Player player) {
		...
		for (WaterCombo combo : instances)
			if (combo.player != null && combo.player == player) list.add(combo);
		return list;
	}
	...
	public static ArrayList<WaterCombo> getWaterCombo(Player player, String ability) {
		ArrayList<WaterCombo> list = new ArrayList<WaterCombo>();
		for (WaterCombo combo : instances)
			if (combo.player != null && combo.player == player && ability != null && combo.ability.equalsIgnoreCase(ability)) list.add(combo);
		return list;
	}
