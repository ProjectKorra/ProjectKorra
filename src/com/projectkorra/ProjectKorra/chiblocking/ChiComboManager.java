...
public class ChiComboManager {
	...
	public static void addCombo(Player player, ChiCombo combo) {
		if (!player.hasPermission("bending.ability.ChiCombo")) return;

		if (!instances.containsKey(player)) instances.put(player, new ArrayList<ChiCombo>());
		instances.get(player).add(combo);

		if (instances.get(player).size() > 4) instances.put(player, shiftList(instances.get(player)));
