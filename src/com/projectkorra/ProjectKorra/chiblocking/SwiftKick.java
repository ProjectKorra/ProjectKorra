...
public class SwiftKick {
	...
	public boolean isEligible(Player player) {
		if (!GeneralMethods.canBend(player.getName(), "SwiftKick")) return false;

		if (GeneralMethods.getBoundAbility(player) == null) return false;

		if (!GeneralMethods.getBoundAbility(player).equalsIgnoreCase("SwiftKick")) return false;

		if (GeneralMethods.isRegionProtectedFromBuild(player, "SwiftKick", player.getLocation())) return false;

		if (GeneralMethods.getBendingPlayer(player.getName()).isOnCooldown("SwiftKick")) return false;
