...
public class AbilityModuleManager {
    ...
    private void fill() {
        ...
        for (StockAbilities a : StockAbilities.values()) {
            if (StockAbilities.isAirbending(a)) {
                ...
                if (ProjectKorra.plugin.getConfig().getBoolean("Abilities.Earth." + a.name() + ".Enabled")) {
                    ...
                    // if (a == StockAbilities.LavaSurge)
                    // earthsubabilities.add(a.name());
        ...
        for (AbilityModule ab : ability) {
            try {
                // To check if EarthBlast == Earthblast or for example,
                // EarthBlast == EARTHBLAST
                boolean succes = true;
                ...
                // if (ab.isMetalbendingAbility())
                // metalbendingabilities.add(ab.getName());
                descriptions.put(ab.getName(), ab.getDescription());
                authors.put(ab.getName(), ab.getAuthor());
            } catch (AbstractMethodError e) { // If triggered means ability was
                                              // made pre 1.6 BETA 8
