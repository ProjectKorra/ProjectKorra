...
public class LavaFlow {
	...
	private long clickLavaDelay, clickLandDelay, clickLavaCooldown,
			clickLandCooldown, shiftCooldown;
	 ...
	 * 
	 * @param player
	 *            the player that bended the ability
	 * @param type
	 *            either shift or sneak
	 */
	public LavaFlow(Player player, AbilityType type) {
		if (!EarthMethods.canLavabend(player)) return;
		...
		if (type == AbilityType.SHIFT) {
			...
			if (makeLava) {
				if (bplayer.isOnCooldown("lavaflowmakelava")) {
					remove();
					return;
				} else bplayer.addCooldown("lavaflowmakelava", cooldown);
			}
			...
			if (!makeLava) {
				if (bplayer.isOnCooldown("lavaflowmakeland")) {
					remove();
					return;
				} else bplayer.addCooldown("lavaflowmakeland", cooldown);
			}
			instances.add(this);
		}
	}
	...
	public void progress() {
		...
		if (type == AbilityType.SHIFT) {
			...
			for (double x = -currentRadius; x <= currentRadius + PARTICLE_OFFSET; x++) {
				for (double z = -currentRadius; z < currentRadius + PARTICLE_OFFSET; z++) {
					...
					if (!isLava(block) && dSquared > Math.pow(shiftPlatformRadius, 2)) {
						if (dSquared < Math.pow(currentRadius, 2) && !GeneralMethods.isRegionProtectedFromBuild(player, "LavaFlow", block.getLocation())) {
							if (dSquared < shiftPlatformRadius * 4 || getAdjacentLavaBlocks(block.getLocation()).size() > 0) createLava(block);
				...
				if (currentRadius > shiftMaxRadius) currentRadius = shiftMaxRadius;
			}
		}
			...
			if (makeLava && curTime > clickLavaCleanupDelay) {
				...
				for (double x = -clickLavaRadius; x <= clickLavaRadius; x++)
					for (double z = -clickLavaRadius; z <= clickLavaRadius; z++) {
						...
						if (tempBlock != null && !isLava(tempBlock) && Math.random() < PARTICLE_DENSITY && tempBlock.getLocation().distanceSquared(origin) <= Math.pow(clickLavaRadius, 2)) ParticleEffect.LAVA.display(loc, 0, 0, 0, 0, 1);
					}
				return;
			}
					...
					for (double z = -radius; z <= radius; z++) {
						...
						if (dSquared < Math.pow(radius, 2) && !GeneralMethods.isRegionProtectedFromBuild(player, "LavaFlow", loc)) {
							if (makeLava && !isLava(tempBlock)) {
								clickIsFinished = false;
								if (Math.random() < lavaCreateSpeed) createLava(tempBlock);
								else ParticleEffect.LAVA.display(loc, 0, 0, 0, 0, 1);
							} else if (!makeLava && isLava(tempBlock)) {
								clickIsFinished = false;
								if (Math.random() < landCreateSpeed) removeLava(tempBlock);
							}
						}
					}
				return;
	 ...
	 * 
	 * @param block
	 *            the block that will be turned to lava
	 */
	 ...
	 * 
	 * @param testBlock
	 *            the block to attempt to remove
	 */
	...
	public static ArrayList<Block> getAdjacentBlocks(Location loc) {
		...
		for (int x = -1; x <= 1; x++)
			for (int y = -1; y <= 1; y++)
				for (int z = -1; z <= 1; z++)
					if (!(x == 0 && y == 0 && z == 0)) list.add(block.getLocation().add(x, y, z).getBlock());
		return list;
	}
	 ...
	 * 
	 * @param loc
	 *            the middle location of the adjacent blocks
	...
	public static boolean isEarthbendableMaterial(Material mat, Player player) {
		for (String s : ProjectKorra.plugin.getConfig().getStringList("Properties.Earth.EarthbendableBlocks"))
			if (mat == Material.getMaterial(s)) return true;
	 ...
	 * 
	 * @param player
	 *            the player that created the ability instance
	...
	public static ArrayList<LavaFlow> getLavaFlow(Player player) {
		...
		for (LavaFlow lf : instances)
			if (lf.player != null && lf.player == player) list.add(lf);
		return list;
	}
	 ...
	 *            the player that created the ability instance
	 * @param type
	 *            the specific type of ability we are looking for
	 * @return a list of all the LavaFlow instances
	 */
	public static ArrayList<LavaFlow> getLavaFlow(Player player, AbilityType type) {
		ArrayList<LavaFlow> list = new ArrayList<LavaFlow>();
		for (LavaFlow lf : instances)
			if (lf.player != null && lf.player == player && lf.type != null && lf.type == type) list.add(lf);
		return list;
	}
	...
	public void setClickLavaCooldown(long clickLavaCooldown) {
		...
		if (player != null) bplayer.addCooldown("lavaflowmakelava", clickLavaCooldown);
	}
	...
	public void setClickLandCooldown(long clickLandCooldown) {
		...
		if (player != null) bplayer.addCooldown("lavaflowmakeland", clickLandCooldown);
	}
	...
	public void setShiftCooldown(long shiftCooldown) {
		...
		if (player != null) bplayer.addCooldown("lavaflowshift", shiftCooldown);
	}
