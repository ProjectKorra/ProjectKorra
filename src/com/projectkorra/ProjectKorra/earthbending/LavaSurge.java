...
public class LavaSurge {
    ...
    public boolean isEligible() {
        ...
        return GeneralMethods.canBend(player.getName(), "LavaSurge")
                && GeneralMethods.getBoundAbility(player) != null
                && GeneralMethods.getBoundAbility(player).equalsIgnoreCase("LavaSurge")
                && !GeneralMethods.isRegionProtectedFromBuild(player, "LavaSurge", player.getLocation())
                && EarthMethods.canLavabend(player)
                && !bplayer.isOnCooldown("LavaSurge");
    ...
    public boolean isLava(Block b) {
        return b.getType() == Material.STATIONARY_LAVA || b.getType() == Material.LAVA;
    }
    ...
    public void openFracture() {
        ...
        fracture.addAll(affectedBlocks.stream()
                .filter(b -> EarthMethods.isEarthbendable(player, b))
    ...
    public void revertFracture() {
        fracturetb.forEach(TempBlock::revertBlock);
    ...
    public boolean canMoveThrough(Block block) {
        return EarthMethods.isTransparentToEarthbending(player, startLocation.getBlock()) ||
    ...
    public void removeLava() {
        lava.values().forEach(TempBlock::revertBlock);
    ...
    public void progress() {
        ...
        if (surgeStarted) {
            ...
            if (curTime > time + (fallingBlockInterval * fallingBlocksCount)) {
                ...
                fracture.stream()
                        .filter(b -> randy.nextBoolean() && b != sourceBlock)
                        .forEach(b -> {
            ...
            for (FallingBlock fb : fblocks) {
                GeneralMethods.getEntitiesAroundPoint(fb.getLocation(), 2).stream()
                        .filter(e -> e instanceof LivingEntity)
                        .filter(e -> e.getEntityId() != player.getEntityId())
                        .forEach(e -> {
