...
public class LavaSurge {
	...
	public LavaSurge(Player player) {
		...
		if (GeneralMethods.getBendingPlayer(player.getName()).isOnCooldown("LavaSurge")) return;
	...
	public boolean isEligible() {
		...
		if (!GeneralMethods.canBend(player.getName(), "LavaSurge")) return false;

		if (GeneralMethods.getBoundAbility(player) == null) return false;

		if (!GeneralMethods.getBoundAbility(player).equalsIgnoreCase("LavaSurge")) return false;

		if (GeneralMethods.isRegionProtectedFromBuild(player, "LavaSurge", player.getLocation())) return false;

		if (!EarthMethods.canLavabend(player)) return false;

		if (bplayer.isOnCooldown("LavaSurge")) return false;
	...
	public boolean prepare() {
		...
		if (targetBlock == null || !(targetBlock.getRelative(BlockFace.UP).getType() == Material.AIR) && !isLava(targetBlock.getRelative(BlockFace.UP))) return false;

		if (instances.containsKey(player)) instances.get(player).revertFracture();

		if ((canSourceBeEarth && EarthMethods.isEarthbendable(player, targetBlock)) || EarthMethods.isLavabendable(targetBlock, player)) {
	...
	public boolean isLava(Block b) {
		if (b.getType() == Material.STATIONARY_LAVA || b.getType() == Material.LAVA) return true;
		return false;
	}
	...
	public void launch() {
		...
		if (canSourceBeEarth) openFracture();
		else skipFracture();
	}
	...
	public boolean canMoveThrough(Block block) {
		if (EarthMethods.isTransparentToEarthbending(player, startLocation.getBlock()) || EarthMethods.isEarthbendable(player, startLocation.getBlock()) || EarthMethods.isLavabendable(startLocation.getBlock(), player)) return true;
		return false;
	}
	...
	public void progress() {
		...
		if (!surgeStarted && sourceBlock != null && curTime > lastTime + particleInterval) {
			lastTime = curTime;
		...
		if (fractureOpen && !surgeStarted) {
			if (!li.hasNext()) surgeStarted = true;
