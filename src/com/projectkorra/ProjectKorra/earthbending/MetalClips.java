...
public class MetalClips {
	...
	public boolean isEligible() {
		...
		if (!GeneralMethods.canBend(player.getName(), "MetalClips")) return false;

		if (GeneralMethods.getBoundAbility(player) == null) return false;

		if (!GeneralMethods.getBoundAbility(player).equalsIgnoreCase("MetalClips")) return false;

		if (GeneralMethods.isRegionProtectedFromBuild(player, "MetalClips", player.getLocation())) return false;

		if (!EarthMethods.canMetalbend(player)) return false;

		if (bplayer.isOnCooldown("MetalClips")) return false;
	...
	public void shootMetal() {
		...
		if (GeneralMethods.getBendingPlayer(player.getName()).isOnCooldown("MetalClips")) return;
		...
		if (GeneralMethods.getTargetedEntity(player, 10, new ArrayList<Entity>()) != null) v = GeneralMethods.getDirection(player.getLocation(), GeneralMethods.getTargetedEntity(player, 10, new ArrayList<Entity>()).getLocation());
		else v = GeneralMethods.getDirection(player.getLocation(), GeneralMethods.getTargetedLocation(player, 10));
	...
	public void formArmor() {
		if (metalclips >= 4) return;

		if (metalclips == 3 && !player.hasPermission("bending.ability.MetalClips.4clips")) return;

		metalclips = (metalclips < 4) ? metalclips + 1 : 4;

		if (target instanceof Player) {
			Player target = (Player) this.target;
			if (oldarmor == null) oldarmor = target.getInventory().getArmorContents();
			...
			if (oldarmor == null) oldarmor = target.getEquipment().getArmorContents();
	...
	public void resetArmor() {
		if (target == null || oldarmor == null) return;

		if (target instanceof Player) ((Player) target).getInventory().setArmorContents(oldarmor);
		else target.getEquipment().setArmorContents(oldarmor);
