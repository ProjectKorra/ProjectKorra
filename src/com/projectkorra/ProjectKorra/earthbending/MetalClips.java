...
public class MetalClips {
    ...
    public boolean isEligible() {
        ...
        return GeneralMethods.canBend(player.getName(), "MetalClips")
                && GeneralMethods.getBoundAbility(player) != null
                && GeneralMethods.getBoundAbility(player).equalsIgnoreCase("MetalClips")
                && !GeneralMethods.isRegionProtectedFromBuild(player, "MetalClips", player.getLocation())
                && EarthMethods.canMetalbend(player)
                && !bplayer.isOnCooldown("MetalClips");
    ...
    public void shootMetal() {
        ...
        if (GeneralMethods.getTargetedEntity(player, 10, new ArrayList<>()) != null)
            v = GeneralMethods.getDirection(player.getLocation(), GeneralMethods.getTargetedEntity(player, 10, new ArrayList<>()).getLocation());
        else
    ...
    public void progress() {
        ...
        for (int i = 0; i < trackedIngots.size(); i++) {
            ...
            if (ii.getItemStack().getType() == Material.IRON_INGOT) {
                ...
                GeneralMethods.getEntitiesAroundPoint(ii.getLocation(), 2).stream()
                        .filter(e -> e instanceof LivingEntity && e.getEntityId() != player.getEntityId())
                        .forEach(e -> {
    ...
    public void remove() {
        trackedIngots.forEach(Item::remove);
        resetArmor();
        trackedIngots.clear();
