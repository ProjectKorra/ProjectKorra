...
public class EarthSmash {
	...
	private double grabRange, chargeTime, damage, knockback, knockup, flySpeed,
			shootRange;
	...
	public EarthSmash(Player player, ClickType type) {
		if (!GeneralMethods.hasPermission(player, "EarthSmash")) return;
	...
	public void progress() {
		...
		if (state == State.START && progressCounter > 1) {
			...
			if (System.currentTimeMillis() - delay >= SHOOTING_ANIMATION_COOLDOWN) {
				...
				for (Block block : getBlocks())
					if (block.getType() != Material.AIR && (!EarthMethods.isTransparentToEarthbending(player, block) || block.getType() == Material.WATER || block.getType() == Material.STATIONARY_WATER)) badBlocksFound++;
		...
		for (int x = -1; x <= 1; x++)
			for (int y = -1; y <= 1; y++)
				for (int z = -1; z <= 1; z++)
					...
					if (loc != null) blocks.add(loc.getWorld().getBlockAt(loc.clone().add(x, y, z)));
		return blocks;
	}
	...
	private EarthSmash aimingAtSmashCheck(Player player, State reqState) {
		...
		for (EarthSmash smash : instances) {
			if (reqState == null || smash.state == reqState) for (Block block : blocks)
				if (block.getLocation().getWorld() == smash.loc.getWorld() && block.getLocation().distanceSquared(smash.loc) <= Math.pow(GRAB_DETECTION_RADIUS, 2)) return smash;
		}
		return null;
	}
	...
	public void setCooldown(long cooldown) {
		...
		if (player != null) bplayer.addCooldown("EarthSmash", cooldown);
	}
