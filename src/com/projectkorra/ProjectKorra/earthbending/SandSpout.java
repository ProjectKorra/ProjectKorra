    ...
    private void rotateSandColumn(Block block) {
        ...
        if (System.currentTimeMillis() >= time + interval) {
            ...
            for (int i = 1; i <= dy; i++) {
                ...
                if (((block.getType() == Material.SAND && block.getData() == (byte) 0) || block.getType() == Material.SANDSTONE)) {
                    EarthMethods.displaySandParticle(effectloc2, 1f, 3f, 1f, 20, .2f, false);
                } else if (((block.getType() == Material.SAND && block.getData() == (byte) 1) || block.getType() == Material.RED_SANDSTONE)) {
                ...
                if (!players.isEmpty())
                    players.stream()
                            .filter(sPlayer -> !sPlayer.equals(player))
                            .forEach(sPlayer -> {
