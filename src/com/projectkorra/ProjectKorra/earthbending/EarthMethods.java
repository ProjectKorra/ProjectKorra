...
public class EarthMethods {
	 ...
	 * 
	 * @param player
	 *            the player that is attempting to Earthbend.
	 * @param range
	 *            the maximum block selection range.
	...
	public static Block getEarthSourceBlock(Player player, double range) {
		...
		for (double i = 0; i <= range; i++) {
			...
			if (GeneralMethods.isRegionProtectedFromBuild(player, "RaiseEarth", location)) continue;
	 ...
	 * 
	 * @param loc
	 *            the initial location to search from.
	 * @param radius
	 *            the maximum radius to search for the earth block.
	 * @param maxVertical
	...
	public static Block getLavaSourceBlock(Player player, double range) {
		...
		for (double i = 0; i <= range; i++) {
			Block block = location.clone().add(vector.clone().multiply(i)).getBlock();
			if (GeneralMethods.isRegionProtectedFromBuild(player, "LavaSurge", location)) continue;
	...
	public static boolean isMetalBlock(Block block) {
		if (block.getType() == Material.GOLD_BLOCK || block.getType() == Material.IRON_BLOCK || block.getType() == Material.IRON_ORE || block.getType() == Material.GOLD_ORE || block.getType() == Material.QUARTZ_BLOCK || block.getType() == Material.QUARTZ_ORE) return true;
		return false;
	}
	...
	public static boolean isTransparentToEarthbending(Player player, String ability, Block block) {
		if (!Arrays.asList(transparentToEarthbending).contains(block.getTypeId())) return false;
		if (!GeneralMethods.isRegionProtectedFromBuild(player, ability, block.getLocation())) return true;
		return false;
	}
	...
	public static boolean moveEarth(Player player, Block block, Vector direction, int chainlength, boolean throwplayer) {
		if (isEarthbendable(player, block) && !GeneralMethods.isRegionProtectedFromBuild(player, "RaiseEarth", block.getLocation())) {
			...
			if (affectedblock == null) return false;
				...
				if (throwplayer) {
					for (Entity entity : GeneralMethods.getEntitiesAroundPoint(affectedblock.getLocation(), 1.75)) {
						if (entity instanceof LivingEntity) {
							...
							if (lentity.getEyeLocation().getBlockX() == affectedblock.getX() && lentity.getEyeLocation().getBlockZ() == affectedblock.getZ()) if (!(entity instanceof FallingBlock)) entity.setVelocity(norm.clone().multiply(.75));
						} else {
							if (entity.getLocation().getBlockX() == affectedblock.getX() && entity.getLocation().getBlockZ() == affectedblock.getZ()) if (!(entity instanceof FallingBlock)) entity.setVelocity(norm.clone().multiply(.75));
						}
					}
	...
	public static void removeRevertIndex(Block block) {
		if (movedearth.containsKey(block)) {
			Information info = movedearth.get(block);
			if (block.getType() == Material.SANDSTONE && info.getType() == Material.SAND) block.setType(Material.SAND);
			if (EarthColumn.blockInAllAffectedBlocks(block)) EarthColumn.revertBlock(block);
	...
	public static void revertAirBlock(int i, boolean force) {
		if (!tempair.containsKey(i)) return;
	...
	public static boolean revertBlock(Block block) {
		...
		if (movedearth.containsKey(block)) {
			...
			if (block.equals(sourceblock)) {
				info.getState().update(true);
				if (EarthColumn.blockInAllAffectedBlocks(sourceblock)) EarthColumn.revertBlock(sourceblock);
				if (EarthColumn.blockInAllAffectedBlocks(block)) EarthColumn.revertBlock(block);
