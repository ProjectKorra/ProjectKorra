...
public class Commands {
	...
	public Commands(ProjectKorra plugin) {
		this.plugin = plugin;
		debug = ProjectKorra.plugin.getConfig().getBoolean("debug");
		init();
	}
	...
	private void init() {
        ...
        CommandExecutor exe = (s, c, label, args) -> {
            ...
            if (Arrays.asList(bindaliases).contains(args[0])) {
                ...
                if (args.length == 2) {
                    ...
                    if (!GeneralMethods.canBind(s.getName(), ability)) {
                        s.sendMessage(ChatColor.RED + "You don't have permission to do that.");
                        return true;
...
//						}
            ...
            if (Arrays.asList(displayaliases).contains(args[0])) {
                ...
                if (args.length == 2) {
                    ...
                    if (Arrays.asList(flightaliases).contains(args[1])) {
                        ...
                        AbilityModuleManager.flightabilities.stream()
                                .filter(st -> GeneralMethods.canView((Player) s, st))
                                .forEach(st -> s.sendMessage(GeneralMethods.getSubBendingColor(Element.Air) + st));
                        return true;
                    }
                    ...
                    if (Arrays.asList(spiritualprojectionaliases).contains(args[1])) {
                        ...
                        AbilityModuleManager.spiritualprojectionabilities.stream()
                                .filter(st -> GeneralMethods.canView((Player) s, st))
                                .forEach(st -> s.sendMessage(GeneralMethods.getSubBendingColor(Element.Air) + st));
                        return true;
                    }
                    ...
                    if (Arrays.asList(bloodaliases).contains(args[1])) {
                        ...
                        AbilityModuleManager.bloodabilities.stream()
                                .filter(st -> GeneralMethods.canView((Player) s, st))
                                .forEach(st -> s.sendMessage(GeneralMethods.getSubBendingColor(Element.Water) + st));
                        return true;
                    }
                    ...
                    if (Arrays.asList(healingaliases).contains(args[1])) {
                        ...
                        AbilityModuleManager.healingabilities.stream()
                                .filter(st -> GeneralMethods.canView((Player) s, st))
                                .forEach(st -> s.sendMessage(GeneralMethods.getSubBendingColor(Element.Water) + st));
                        return true;
                    }
                    ...
                    if (Arrays.asList(icealiases).contains(args[1])) {
                        ...
                        AbilityModuleManager.iceabilities.stream()
                                .filter(st -> GeneralMethods.canView((Player) s, st))
                                .forEach(st -> s.sendMessage(GeneralMethods.getSubBendingColor(Element.Water) + st));
                    ...
                    if (Arrays.asList(plantaliases).contains(args[1])) {
                        ...
                        AbilityModuleManager.plantabilities.stream()
                                .filter(st -> GeneralMethods.canView((Player) s, st))
                                .forEach(st -> s.sendMessage(GeneralMethods.getSubBendingColor(Element.Water) + st));
                        return true;
                    }
                    ...
                    if (Arrays.asList(lavabendingaliases).contains(args[1])) {
                        ...
                        AbilityModuleManager.lavaabilities.stream()
                                .filter(st -> GeneralMethods.canView((Player) s, st))
                                .forEach(st -> s.sendMessage(GeneralMethods.getSubBendingColor(Element.Earth) + st));
                        return true;
                    }
                    ...
                    if (Arrays.asList(metalbendingaliases).contains(args[1])) {
                        ...
                        AbilityModuleManager.metalabilities.stream()
                                .filter(st -> GeneralMethods.canView((Player) s, st))
                                .forEach(st -> s.sendMessage(GeneralMethods.getSubBendingColor(Element.Earth) + st));
                        return true;
                    }
                    ...
                    if (Arrays.asList(sandbendingaliases).contains(args[1])) {
                        ...
                        AbilityModuleManager.sandabilities.stream()
                                .filter(st -> GeneralMethods.canView((Player) s, st))
                                .forEach(st -> s.sendMessage(GeneralMethods.getSubBendingColor(Element.Earth) + st));
                        return true;
                    }
                    ...
                    if (Arrays.asList(lightningaliases).contains(args[1])) {
                        ...
                        AbilityModuleManager.lightningabilities.stream()
                                .filter(st -> GeneralMethods.canView((Player) s, st))
                                .forEach(st -> s.sendMessage(GeneralMethods.getSubBendingColor(Element.Fire) + st));
                        return true;
                    }
                    ...
                    if (Arrays.asList(combustionaliases).contains(args[1])) {
                        ...
                        AbilityModuleManager.combustionabilities.stream()
                                .filter(st -> GeneralMethods.canView((Player) s, st))
                                .forEach(st -> s.sendMessage(GeneralMethods.getSubBendingColor(Element.Fire) + st));
                        return true;
                    }
                    ...
                    if (Arrays.asList(chialiases).contains(args[1])) {
                        ...
                        AbilityModuleManager.chiabilities.stream()
                                .filter(st -> GeneralMethods.canView((Player) s, st))
                                .forEach(st -> s.sendMessage(ChiMethods.getChiColor() + st));
                        return true;
                    } else {
            ...
            if (Arrays.asList(importaliases).contains(args[0])) {
                ...
                final LinkedList<BendingPlayer> bPlayers = new LinkedList<>();
                for (String string : bendingPlayers.getConfigurationSection("").getKeys(false)) {
                    ...
                    element.addAll(oe.stream()
                            .filter(i -> Element.getType(i) != null)
                            .map(Element::getType)
                            .collect(Collectors.toList()));

                    BendingPlayer bPlayer = new BendingPlayer(uuid, string, element, abilities, permaremoved);
                ...
                importTask = Bukkit.getServer().getScheduler().runTaskTimerAsynchronously(plugin, () -> {
                    int i = 0;
                    if (i >= 10) {
            ...
            if (Arrays.asList(presetaliases).contains(args[0])) {
                ...
                if (args.length == 2 && Arrays.asList(listaliases).contains(args[1])) {
                    ...
                    presetNames.addAll(presets.stream()
                            .map(Preset::getName)
                            .collect(Collectors.toList()));
