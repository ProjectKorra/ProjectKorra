...
public class HeatControl {
	...
	public boolean isEligible(Player player) {
		if (!GeneralMethods.canBend(player.getName(), "HeatControl")) return false;

		if (GeneralMethods.getBoundAbility(player) == null) return false;

		if (!GeneralMethods.getBoundAbility(player).equalsIgnoreCase("HeatControl")) return false;
	...
	public void freeze(List<Location> area) {
		if (System.currentTimeMillis() < lastBlockTime + delay) return;

		List<Block> lava = new ArrayList<Block>();

		for (Location l : area)
			if (EarthMethods.isLava(l.getBlock())) lava.add(l.getBlock());
		...
		if (!tblocks.contains(tb)) tblocks.add(tb);

	}

	public void particles(List<Location> area) {
		if (System.currentTimeMillis() < lastParticleTime + 300) return;
