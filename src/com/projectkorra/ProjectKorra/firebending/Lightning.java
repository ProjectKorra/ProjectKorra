...
public class Lightning {
	...
	private double range, chargeTime, cooldown, subArcChance, damage,
			chainArcs, chainRange, waterRange;
	 ...
	 * 
	 * @param player
	 *            the player that is viewing the block
	 * @param block
	 *            the block
	...
	public boolean isTransparent(Player player, Block block) {
		if (Arrays.asList(EarthMethods.transparentToEarthbending).contains(block.getTypeId())) {
			if (GeneralMethods.isRegionProtectedFromBuild(player, "Lightning", block.getLocation())) return false;
	 ...
	 * 
	 * @param lent
	 *            : the LivingEntity that is being damaged
	 */
	...
	public static Lightning getLightning(Player player) {
		for (Lightning light : instances) {
			if (light.player == player) return light;
		}
		return null;
	}
		...
		public void run() {
			...
			if (count > 5) this.cancel();
				...
				for (Entity entity : GeneralMethods.getEntitiesAroundPoint(loc, 2.5)) {
					 ...
					 */
					if (entity.equals(player) && !(SELF_HIT_WATER && hitWater && isWater(player.getLocation())) && !(SELF_HIT_WATER && hitIce)) continue;
	...
	public void setCooldown(double cooldown) {
		...
		if (player != null) bplayer.addCooldown("Lightning", (long) cooldown);
	}
