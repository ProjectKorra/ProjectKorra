...
public class Lightning {
    ...
    public boolean isTransparent(Player player, Block block) {
        return Arrays.asList(EarthMethods.transparentToEarthbending).contains(block.getTypeId())
                && !GeneralMethods.isRegionProtectedFromBuild(player, "Lightning", block.getLocation())
                && (!isIce(block.getLocation())
                || ARC_ON_ICE);
    }
    ...
    public static void progressAll() {
        instances.forEach(Lightning::progress);
    }
    ...
    public class Arc {
        ...
        public void generatePoints(int times) {
            ...
            points.forEach(point -> {
                animLocs.add(new AnimLocation(point, animCounter));
                animCounter++;
            });
        }
        ...
        public ArrayList<Arc> generateArcs(double chance, double range) {
            ...
            animLocs.stream()
                    .filter(animLoc -> Math.random() < chance)
                    .forEach(animLoc -> {
                        Location loc = animLoc.getLoc();
                        ...
                        subArcs.add(arc);
                        arc.setAnimCounter(animLoc.getAnimCounter());
         ...
         */
        public void cancel() {
            particles.forEach(Lightning.LightningParticle::cancel);
            subArcs.forEach(Lightning.Arc::cancel);
        }
    ...
    public static ArrayList<Arc> getAllArcs() {
        ...
        for (Lightning l : Lightning.instances) {
            a.addAll(l.getArcs().stream().collect(Collectors.toList()));
        }
        return a;
    }
}