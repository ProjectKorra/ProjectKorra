...
public class FireShield {
	...
	public FireShield(Player player, boolean shield) {
		...
		if (!player.getEyeLocation().getBlock().isLiquid()) {
			...
			if (!shield) bPlayer.addCooldown("FireShield", GeneralMethods.getGlobalCooldown());
		}
	}
			...
			if (shield) {
				...
				for (double theta = 0; theta < 180; theta += 20) {
					for (double phi = 0; phi < 360; phi += 20) {
						...
						if (!blocks.contains(block) && !GeneralMethods.isSolid(block) && !block.isLiquid()) blocks.add(block);
					}
				}
				...
				for (Entity entity : GeneralMethods.getEntitiesAroundPoint(location, radius)) {
					if (GeneralMethods.isRegionProtectedFromBuild(player, "FireShield", entity.getLocation())) continue;
				...
				for (Block block : blocks) {
					if (!GeneralMethods.isRegionProtectedFromBuild(player, "FireShield", block.getLocation())) {
						ParticleEffect.FLAME.display(block.getLocation(), 0.6F, 0.6F, 0.6F, 0, 7);
