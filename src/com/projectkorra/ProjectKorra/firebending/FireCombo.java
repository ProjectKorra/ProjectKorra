...
{	
	...
	public FireCombo(Player player, String ability)
	{
		if(!enabled || !player.hasPermission("bending.ability.FireCombo"))
			return;
		if(Methods.isRegionProtectedFromBuild(player, "Blaze", player.getLocation()))
			return;
	...
	public void progress()
	{
		...
		if(ability.equalsIgnoreCase("FireKick"))
		{
			if(destination == null)
			{
				...
				for(int i = -30; i <= 30; i+=5)
				{
					...
					tasks.add(fs);player.getWorld().playSound(player.getLocation(), Sound.FIRE_IGNITE, 0.5f, 1f);
				}
		...
		else if(ability.equalsIgnoreCase("FireWheel"))
		{
			...
			if(topBlock == null || 
					(Methods.isWaterbendable(topBlock, player) && !Methods.isPlant(topBlock))){
				remove();
				return;
			}
	...
	public void checkSafeZone(){
		if(currentLoc != null && Methods.isRegionProtectedFromBuild(player, "Blaze", currentLoc))
			remove();
	}
	public void collision(LivingEntity entity, Vector direction, FireComboStream fstream)
	{
		if(Methods.isRegionProtectedFromBuild(player, "Blaze", entity.getLocation()))
			return;
		...
		{
			double knockback = AvatarState.isAvatarState(player) ? 
					FIRE_SPIN_KNOCKBACK + 0.5 : FIRE_SPIN_KNOCKBACK;
			Methods.damageEntity(player, entity, damage);
	...
	{
		/**
		 * Removes this instance of FireCombo, cleans up
	...
	public class FireComboStream extends BukkitRunnable
	{
		...
		public void run() 
		{
			...
			{
				for(Entity entity : Methods.getEntitiesAroundPoint(currentLoc, collisionRadius))
				{
