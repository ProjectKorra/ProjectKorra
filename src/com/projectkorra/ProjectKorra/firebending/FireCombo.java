...
public class FireCombo {
	...
	public static double fireticksFireWheel = ProjectKorra.plugin.getConfig().getDouble("Abilities.Fire.FireCombo.FireWheel.FireTicks");
	public static double fireticksJetBlaze = ProjectKorra.plugin.getConfig().getDouble("Abilities.Fire.FireCombo.JetBlaze.FireTicks");
	...
	public FireCombo(Player player, String ability) {
		...
		if (GeneralMethods.isRegionProtectedFromBuild(player, "Blaze", player.getLocation())) return;
		...
		if (ability.equalsIgnoreCase("FireKick")) {
			if (destination == null) {
				...
				for (FireComboStream stream : tasks)
					if (GeneralMethods.blockAbilities(player, abilitiesToBlock, stream.currentLoc, 2)) stream.remove();
			...
			for (FireComboStream stream : tasks) {
				if (FireMethods.isWithinFireShield(stream.getLocation())) stream.remove();
				if (AirMethods.isWithinAirShield(stream.getLocation())) stream.remove();
			}
		} else if (ability.equalsIgnoreCase("JetBlast")) {
			...
			if (topBlock.getType() == Material.FIRE || WaterMethods.isPlant(topBlock)) topBlock = topBlock.getLocation().add(0, -1, 0).getBlock();
	...
	public void checkSafeZone() {
		if (currentLoc != null && GeneralMethods.isRegionProtectedFromBuild(player, "Blaze", currentLoc)) remove();
	}

	public void collision(LivingEntity entity, Vector direction, FireComboStream fstream) {
		if (GeneralMethods.isRegionProtectedFromBuild(player, "Blaze", entity.getLocation())) return;
	...
	public static class FireComboStream extends BukkitRunnable {
		...
		public void run() {
			...
			for (int i = 0; i < density; i++) {
				if (useNewParticles) particleEffect.display(currentLoc, spread, spread, spread, 0, 1);
				else currentLoc.getWorld().playEffect(currentLoc, Effect.MOBSPAWNER_FLAMES, 0, 15);
			}
			...
			if (initialLoc.distance(currentLoc) > distance) {
				...
				for (Entity entity : GeneralMethods.getEntitiesAroundPoint(currentLoc, collisionRadius)) {
					if (entity instanceof LivingEntity && !entity.equals(fireCombo.getPlayer())) fireCombo.collision((LivingEntity) entity, direction, this);
				}
			}
			checkCollisionCounter++;
			if (singlePoint) remove();
		}
