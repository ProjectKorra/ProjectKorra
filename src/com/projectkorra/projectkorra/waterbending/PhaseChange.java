...
public class PhaseChange extends IceAbility {
	...
	/*
	 * Skate Variables private long skateCooldown = 7000; private int
	 * skateRadius = 1; private long duration = 7000; private double speed =
	 * 0.335;
	 */
	...
	public void progress() {
		...
		/*
		 * if (active_types.contains(PhaseChangeType.SKATE)) { if
		 * (!player.isSprinting()) { active_types.remove(PhaseChangeType.SKATE);
		 * bPlayer.addCooldown("PhaseChangeSkate", skateCooldown); return; }
		 * 
		 * if (System.currentTimeMillis() > getStartTime() + duration) { return;
		 * }
		 * 
		 * Location center = player.getLocation().clone().subtract(0, 1, 0); if
		 * (isWater(center.getBlock()) || isIce(center.getBlock())) {
		 * freezeArea(center, skateRadius, PhaseChangeType.SKATE); Vector v =
		 * new Vector(player.getLocation().getDirection().getX(), 0,
	...
	public void setFields(PhaseChangeType type) {
		...
		if (type == PhaseChangeType.FREEZE) {
			...
			/*
			 * } else if (type == PhaseChangeType.SKATE) { if
			 * (bPlayer.isOnCooldown("PhaseChangeSkate")) { return; } duration =
			 * night*getConfig().getLong(
			 * "Abilities.Water.PhaseChange.Skate.Duration"); speed =
			 * night*getConfig().getDouble(
			 * "Abilities.Water.PhaseChange.Skate.Speed"); skateCooldown =
			 * getConfig().getLong("Abilities.Water.PhaseChange.Skate.Cooldown")
			 * ; skateRadius = night*getConfig().getInt(
			 * "Abilities.Water.PhaseChange.Skate.Radius");
			 * 
			 * freezeArea(player.getLocation().clone().subtract(0, 1, 0),
			 * skateRadius, PhaseChangeType.SKATE);
			 */
		}
	}

	/*
	 * public void displaySkateParticles() { Location right =
	 * GeneralMethods.getRightSide(player.getLocation(), 0.3); Location left =
	...
	/*
	 * public int getSkateFreezeRadius() { return skateRadius; }
	 */
