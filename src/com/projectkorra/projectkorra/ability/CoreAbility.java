...
public abstract class CoreAbility implements Ability {
	 ...
	 * 
	 * @param player
	 *            the non-null player that created this instance
	 * @see #start()
	 */
	 ...
	 * 
	 * @param player
	 *            the player that created the CoreAbility instance
	 * @param clazz
	 *            the class of the type of CoreAbility
	 ...
	 * 
	 * @param abilityName
	 *            the name of a loaded CoreAbility
	...
	/**
	 * Returns a list of "fake" instances for each ability that was loaded by
	 ...
	 * 
	 * @param clazz
	 *            the class for the type of CoreAbilities
	 ...
	 * 
	 * @param player
	 *            the player that created the instances
	 * @param clazz
	 *            the class for the type of CoreAbilities
	 * @return a Collection of real instances
	 */
	...
	public static <T extends CoreAbility> Collection<T> getAbilities(Player player, Class<T> clazz) {
		if (player == null || clazz == null || INSTANCES.get(clazz) == null
				|| INSTANCES.get(clazz).get(player.getUniqueId()) == null) {
			return Collections.emptySet();
		}
	 ...
	 * 
	 * @param element
	 *            the Element of the loaded abilities
	 ...
	 * 
	 * @param player
	 *            the player that created the T instance
	 * @param clazz
	 *            the class for the type of CoreAbility
	 */
	 ...
	 * 
	 * @param clazz
	 *            the clazz for the type of CoreAbility
	 */
	 ...
	 * 
	 * @param plugin
	 *            a JavaPlugin containing CoreAbility class files
	 * @param packagePrefix
	 *            a prefix of the package name, used to increase performance
	 * @see #getAbilities()
	 * @see #getAbility(String)
	 */
	public static void registerPluginAbilities(JavaPlugin plugin, String packagePrefix) {
		List<String> disabled = new ArrayList<String>(); // this way multiple
															// classes with the
															// same name only
															// show once
		if (plugin == null) {
		...
		try {
			for (final ClassInfo info : ClassPath.from(loader).getAllClasses()) {
				...
				try {
					clazz = info.load();
					if (!CoreAbility.class.isAssignableFrom(clazz) || clazz.isInterface()
					...
					if (ability instanceof ComboAbility) {
						...
						if (combo.getCombination() != null) {
							ComboManager.getComboAbilities().put(name,
									new ComboManager.ComboAbilityInfo(name, combo.getCombination(), combo));
					...
					if (ability instanceof MultiAbility) {
						...
						MultiAbilityManager.multiAbilityList
								.add(new MultiAbilityInfo(name, multiAbil.getMultiAbilities()));
					}
	 ...
	 * 
	 * @param folder
	 *            the name of the folder to scan
	 * @see #getAbilities()
	 * @see #getAbility(String)
	 */
		...
		for (CoreAbility coreAbil : loadedAbilities) {
			if (!(coreAbil instanceof AddonAbility)) {
				plugin.getLogger().warning(
						coreAbil.getName() + " is an addon ability and must implement the AddonAbility interface");
				continue;
			...
			try {
				...
				plugin.getLogger().warning("The ability " + coreAbil.getName()
						+ " was not able to load, if this message shows again please remove it!");
				e.printStackTrace();
				addon.stop();
	...
	public String getDescription() {
		...
		return ConfigManager.languageConfig.get()
				.getString("Abilities." + elementName + "." + getName() + ".Description");
	}
