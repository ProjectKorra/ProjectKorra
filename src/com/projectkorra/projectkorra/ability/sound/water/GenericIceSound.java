...
public class GenericIceSound extends AbilitySound {

    public static GenericIceSound getSound;

    public GenericIceSound() {
        // Get Ice sound properties from the config
        Configuration config = ConfigManager.getConfig();
        this.volume = config.getDouble("Properties.Water.IceSound.Volume");
        ...
        this.pitch = config.getDouble("Properties.Water.IceSound.Pitch");
        if (Double.compare(pitch, 0.5) < 0) this.pitch = 0.5; // min.
        if (Double.compare(pitch, 2.0) > 0) this.pitch = 2.0; // max.
        String n = config.getString("Properties.Water.IceSound.Sound");
        ...
        if (sound == null) {
            this.sound = Sound.ITEM_FLINTANDSTEEL_USE;
        }
